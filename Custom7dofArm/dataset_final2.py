...
class RobotDatasetGenerator:
    def __init__(self):
        ...
        # Modified DH parameters with correct scaling and offsets
        self.dh_params = [
            [0, np.pi/2, 0.15, np.pi],        # Joint 1 (increased d to include base height)
            [0.435, -np.pi/2, 0, -np.pi/2],   # Joint 2 (modified for correct orientation)
