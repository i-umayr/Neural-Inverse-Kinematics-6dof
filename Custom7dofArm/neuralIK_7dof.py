...
# Custom loss function for periodic joint angles
def joint_angle_loss(y_true, y_pred):
    # Regular MSE
    mse = tf.reduce_mean(tf.square(y_true - y_pred))
    
    # Periodic component for continuous joints
    # Indices of continuous joints (q1, q3, q5, q7)
...
def main():
    ...
    input_features = [f'q{i}_current' for i in range(1, 8)] + \
                    ['x_desired', 'y_desired', 'z_desired', 
                     'roll', 'pitch', 'yaw']
    
    output_features = [f'q{i}_desired' for i in range(1, 8)]
    ...
    # Compile model with fixed initial learning rate
    model.compile(
    ...
    model.save('7dof_ik_model.keras')
    import joblib
    ...
    plt.boxplot([joint_errors[:, i] for i in range(7)],
